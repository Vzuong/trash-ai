...
def on_fit_epoch_end(trainer):
    ...
    Reduces learning rate by 50% if validation fitness does not improve for 5 epochs.
    Allows 5 additional epochs to recover before EarlyStopping(patience=10) stops training.
    """
    ...
    if delta == 5 and not getattr(trainer, '_lr_reduced_on_plateau', False):
        ...
        print("\n" + "-" * 75)
        print(f"[ReduceLROnPlateau] No improvement for 5 epochs. Reducing learning rate to: {curr_lr:.6f}")
        print("-" * 75 + "\n")
    elif delta < 5:
...
def train_yolo(cfg):
    ...
    print("-" * 65)
    print("Starting YOLOv11 Trash Recognition Training...")
    print(f"Experiment Name     : {exp_name}")
    print(f"Dataset Config      : {cfg.data_yaml}")
    print(f"Image Size          : {cfg.img_size} | Batch Size: {cfg.batch_size}")
    print(f"Epochs              : {cfg.epochs}")
    print(f"Patience            : {cfg.early_stop}")
    print("-" * 65)
    ...
    try:
        ...
        print(f"[ERROR] Could not load YOLO weights '{cfg.weights}': {e}")
        return
    ...
    if os.path.exists(best_weights):
        ...
        try:
            ...
            val_res = val_model.val(data=cfg.data_yaml, split='test', imgsz=cfg.img_size, device=cfg.gpu, verbose=True)
